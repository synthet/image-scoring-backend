# NEF Folder Processor - Local MUSIQ Models (No TensorFlow Hub)
param(
...
Write-Host "========================================"
Write-Host " NEF Folder Processor - Local Models"
Write-Host " MUSIQ Local Checkpoints Only"
...
Write-Host "Step 1: Processing NEF files with local MUSIQ models..."
Write-Host ""
...
# Run the local batch processor in WSL
$command = "source ~/.venvs/tf/bin/activate && cd /mnt/d/Projects/image-scoring && python scripts/python/batch_process_images_local.py --input-dir '$WSLPath' --output-dir '$WSLPath'"
wsl bash -c $command
