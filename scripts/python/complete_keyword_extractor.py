#!/usr/bin/env python3
"""
Complete AI Keyword Extraction and XMP Sidecar Creation Tool
Extracts keywords using AI and creates XMP sidecar files for NEF files.
"""
...
class CompleteKeywordExtractor:
    """Complete AI-powered keyword extraction with XMP sidecar creation."""
    ...
    def create_xmp_sidecar(self, nef_path: str, keywords: list, caption: str = "") -> bool:
...
<?xpacket end="w"?>'''
            ...
            print(f"Created XMP sidecar with {len(keywords_to_embed)} keywords: {os.path.basename(xmp_path)}")
            return True
    ...
    def extract_keywords(self, image_path: str, create_xmp: bool = False) -> Dict:
        """
        Extract keywords from an image file and optionally create XMP sidecar.
        
        Args:
            image_path: Path to the image file
            create_xmp: Whether to create XMP sidecar file for NEF files
        ...
        try:
            ...
            # Extract keyword strings for XMP
            keyword_strings = [kw["keyword"] for kw in filtered_keywords]
            
            # Create XMP sidecar if requested and it's a NEF file
            xmp_created = False
            if create_xmp and image_path.lower().endswith('.nef'):
                xmp_created = self.create_xmp_sidecar(image_path, keyword_strings, caption)
            ...
            result = {
                ...
                "device": self.device,
                "xmp_sidecar_created": xmp_created,
    ...
    def process_batch(self, image_paths: List[str], create_xmp: bool = False) -> List[Dict]:
        ...
        for i, image_path in enumerate(image_paths, 1):
            ...
            result = self.extract_keywords(image_path, create_xmp)
            ...
            if "error" in result:
                ...
                if create_xmp and result.get('xmp_sidecar_created'):
                    print(f"  XMP sidecar created")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract keywords from images using AI models and create XMP sidecar files",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python complete_keyword_extractor.py --input-dir "D:/Photos/NEF_Files"
  python complete_keyword_extractor.py --input-dir "D:/Photos/NEF_Files" --create-xmp
  python complete_keyword_extractor.py --input-file "D:/Photos/image.nef" --create-xmp
  python complete_keyword_extractor.py --input-dir "D:/Photos" --confidence-threshold 0.05 --create-xmp
        """
    )
    ...
    parser.add_argument('--create-xmp', action='store_true',
                       help='Create XMP sidecar files for NEF files')
    ...
    try:
        extractor = CompleteKeywordExtractor(
            device=args.device,
    ...
    if args.create_xmp:
        ...
        print(f"Will create XMP sidecar files for {len(nef_files)} NEF files")
    
    # Process images
    results = extractor.process_batch(image_files, args.create_xmp)
    ...
    batch_summary = {
        ...
        "xmp_sidecars_created": len([r for r in results if r.get("xmp_sidecar_created", False)]),
        "results": results
    }
    ...
    successful = len([r for r in results if "error" not in r])
    failed = len([r for r in results if "error" in r])
    xmp_created = len([r for r in results if r.get("xmp_sidecar_created", False)])
    ...
    if args.create_xmp:
        print(f"  XMP sidecar files created: {xmp_created}")
