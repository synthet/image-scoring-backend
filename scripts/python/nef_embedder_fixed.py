#!/usr/bin/env python3
"""
NEF Keyword Embedding Utility - Fixed Version
...
def embed_keywords_in_nef(nef_path: str, keywords: list, caption: str = "") -> bool:
    ...
    try:
        ...
        for tag_id, value in exif_dict.items():
            ...
            new_exif_dict[tag] = value
        
        # Add caption to ImageDescription
        if caption:
            new_exif_dict['ImageDescription'] = caption
        
        # Add keywords as a comma-separated string in UserComment
        if keywords_to_embed:
            keywords_string = ', '.join(keywords_to_embed)
            new_exif_dict['UserComment'] = keywords_string
        
        # Save the image with new EXIF data
        image.save(nef_path, exif=piexif.dump(new_exif_dict))
