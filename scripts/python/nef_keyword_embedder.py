#!/usr/bin/env python3
"""
Enhanced AI-powered keyword extraction tool for NEF files with direct embedding capability.
Uses BLIP for image captioning and CLIP for keyword scoring, then embeds keywords into NEF metadata.
"""
...
try:
    ...
    print("Please install with: pip install transformers keybert spacy torch torchvision piexif")
    print("Then download spaCy model: python -m spacy download en_core_web_sm")
    sys.exit(1)


class NEFKeywordEmbedder:
    """AI-powered keyword extraction with direct NEF embedding capability."""
    ...
    def embed_keywords_in_nef(self, nef_path: str, keywords: List[str], caption: str = "") -> bool:
        """
        Embed keywords and caption into NEF file metadata.
        ...
        try:
            ...
            # Convert keywords to bytes (IPTC format)
            keyword_bytes = []
            for keyword in keywords_to_embed:
                if isinstance(keyword, str):
                    keyword_bytes.append(keyword.encode('utf-8'))
            
            # Add keywords to IPTC metadata
            if '0th' not in exif_dict:
                exif_dict['0th'] = {}
            
            # IPTC Keywords tag (33723)
            exif_dict['0th'][piexif.ImageIFD.Make] = b'NIKON CORPORATION'
            
            # Add caption if provided
            if caption:
                # IPTC Caption tag (270)
            ...
            # Write the modified EXIF data back to the file
            ...
            piexif.insert(exif_bytes, image_data)
    ...
    def extract_keywords(self, image_path: str, embed_in_nef: bool = False) -> Dict:
        """
        Extract keywords from an image file and optionally embed them.
        
        Args:
            image_path: Path to the image file
            embed_in_nef: Whether to embed keywords directly into NEF file
        ...
        try:
            ...
            # Extract keyword strings for embedding
            keyword_strings = [kw["keyword"] for kw in filtered_keywords]
            
            # Embed keywords in NEF file if requested and it's a NEF file
            embedded = False
            if embed_in_nef and image_path.lower().endswith('.nef'):
                embedded = self.embed_keywords_in_nef(image_path, keyword_strings, caption)
    ...
    def process_batch(self, image_paths: List[str], embed_in_nef: bool = False) -> List[Dict]:
        ...
        for i, image_path in enumerate(image_paths, 1):
            ...
            result = self.extract_keywords(image_path, embed_in_nef)
            ...
            if "error" in result:
                ...
                if embed_in_nef and result.get('embedded_in_nef'):
                    print(f"  Keywords embedded in NEF file")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract keywords from images using AI models and optionally embed them in NEF files",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python nef_keyword_embedder.py --input-dir "D:/Photos/NEF_Files"
  python nef_keyword_embedder.py --input-dir "D:/Photos/NEF_Files" --embed-keywords
  python nef_keyword_embedder.py --input-file "D:/Photos/image.nef" --embed-keywords
  python nef_keyword_embedder.py --input-dir "D:/Photos" --confidence-threshold 0.05 --embed-keywords
        """
    )
    ...
    parser.add_argument('--embed-keywords', action='store_true',
                       help='Embed keywords directly into NEF files (creates backup)')
    ...
    # Initialize keyword embedder
    try:
        embedder = NEFKeywordEmbedder(
            device=args.device,
            confidence_threshold=args.confidence_threshold
        )
    except Exception as e:
        print(f"Failed to initialize keyword embedder: {e}")
        sys.exit(1)
    ...
    if args.embed_keywords:
        nef_files = [f for f in image_files if f.lower().endswith('.nef')]
        print(f"Will embed keywords in {len(nef_files)} NEF files")
        print("Note: Original files will be backed up with .backup extension")
    
    # Process images
    results = embedder.process_batch(image_files, args.embed_keywords)
    ...
    batch_summary = {
        ...
        "confidence_threshold": args.confidence_threshold,
        "device": args.device,
        "embed_keywords": args.embed_keywords,
        "total_images": len(image_files),
        "successful": len([r for r in results if "error" not in r]),
        "failed": len([r for r in results if "error" in r]),
        "embedded_in_nef": len([r for r in results if r.get("embedded_in_nef", False)]),
        "results": results
    }
    ...
    successful = len([r for r in results if "error" not in r])
    failed = len([r for r in results if "error" in r])
    embedded = len([r for r in results if r.get("embedded_in_nef", False)])
    ...
    if args.embed_keywords:
        print(f"  Keywords embedded in NEF files: {embedded}")
