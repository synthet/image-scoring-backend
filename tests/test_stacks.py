...
def test_db():
    """Set up and tear down a test database for each test function."""
    original_db_path = db.DB_PATH
    
    # Use local temp dir to avoid Firebird permission issues in AppData
    temp_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'temp_test_stack')
    if os.path.exists(temp_dir):
         # Try to clean up previous runs if possible, but don't fail setup
    ...
    try:
        ...
        # Based on inspection, get_db() returns new connections, so we rely on tests closing them
        # But we force GC to close handle references in C extensions
        gc.collect()
    except: pass
...
def _create_test_images():
    ...
    # Create test folder
    c.execute("INSERT INTO folders (path) VALUES (?) RETURNING id", ('D:\\Test\\Photos',))
...
def test_create_stack_from_images_minimum(test_db):
...
def test_path_lookup_fallback(test_db):
