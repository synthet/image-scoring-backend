...
"""
    if use_fallback_pipeline_ui:
        demo = gr.Blocks(title="Image Scoring Runs", head=favicon_links)
    else:
        tree_js = assets.get_tree_js()
        custom_css = assets.get_css()
        demo = gr.Blocks(title="Image Scoring Runs", css=custom_css, head=tree_js + favicon_links)
    ...
    with demo:
        ...
        def monitor_status_wrapper(selected_folder, telemetry_state, collapse_noisy, pin_critical):
            ...
            res = pipeline_tab.get_status_update(
                runner,
                tagging_runner,
                selection_runner,
                orchestrator,
