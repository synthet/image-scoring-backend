...
The frontend adopts a **VS Code Dark+ color system**, deliberately aligned with the Driftara Gallery Electron app. This creates a cohesive, professional, developer-centric, or "pro-tool" aesthetic. The design relies heavily on modern dark mode principles with subtle contrast between surfaces, vibrant interactive accents, and clear semantic feedback colors.

*(For the canonical palette, icon contract, and UI patterns — live connection indicator, score formatting, images table, inspector — see [image-scoring-ui/docs/DESIGN_SYSTEM.md](https://github.com/synthet/image-scoring-ui/blob/main/docs/DESIGN_SYSTEM.md) and the local pointer [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md).)*
